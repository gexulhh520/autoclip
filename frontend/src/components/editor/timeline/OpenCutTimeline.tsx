...
import { clampMainBlockOverlayDropStartSec } from '../../../editor/timeline/mainTrackOverlayMove'
...
const OpenCutTimeline: React.FC<OpenCutTimelineProps> = ({ projectId }) => {
  ...
  const moveMainSequentialBlockToOverlayTrack = useEditSessionStore(
    (state) => state.moveMainSequentialBlockToOverlayTrack
  )
  ...
  const startElementDrag = (
    ...
    if (element.source.kind === 'block') {
      ...
      if (isMainTrackBlock(block) && block.timeline_start_sec == null) {
        ...
        const resolveOverlayDropStartSec = (clientX: number, videoTrackId: string | null) => {
          const proposed = resolveSnappedStartFromClientX(clientX)
          if (!videoTrackId) return proposed
          const session = useEditSessionStore.getState().session
          if (!session) return proposed
          return clampMainBlockOverlayDropStartSec(session, blockId, videoTrackId, proposed)
        }

        const showOverlayDropPreview = (
          overlayTarget: {
          ...
          const previewStart = resolveOverlayDropStartSec(clientX, overlayTarget.videoTrackId)
        ...
        const onMainTrackMove = (moveEvent: PointerEvent) => {
          ...
          if (overlayTarget) {
            showOverlayDropPreview(overlayTarget, moveEvent.clientX)
            return
          }
        ...
        const onMainTrackUp = (upEvent: PointerEvent) => {
          ...
          if (dragMode === 'overlay') {
            ...
            const placementStart = resolveOverlayDropStartSec(upEvent.clientX, overlayTrackId)
            moveMainSequentialBlockToOverlayTrack(blockId, overlayTrackId, placementStart, {
              recordHistory: false,
      ...
      const onVideoMove = (moveEvent: PointerEvent) => {
        ...
        if (isCrossTrack && videoTrack) {
          const previewStart = resolveVideoPlacement(videoTrack.videoTrackId, snapped)
          setSnapPoint({ time: previewStart, type: 'grid' })
          ...
          const placementStart = resolveVideoPlacement(sourceTrack?.videoTrackId, snapped)
          ...
          setBlockDragPreview({
            ...
            fromIndex: 0,
            targetIndex: 0,
            deltaPx: timeToPx(placementStart - initialStart, zoomLevel),
            label: element.name,
            duration: element.duration,
            insertMarkerSec: placementStart,
          })
        }
      }
      ...
      const onVideoUp = (upEvent: PointerEvent) => {
        ...
        if (
          ...
          const placementStart = resolveVideoPlacement(targetVideoTrackId, snapped)
    ...
    const applyMove = (moveEvent: PointerEvent) => {
      ...
      if (element.source.kind === 'overlay') {
        ...
        for (const [overlayId, start] of groupOverlayStarts) {
          ...
          let placement = isCrossTrackPreview ? start : proposedStart
          if (!isCrossTrackPreview && liveSession && sourceTrack?.textTrackId) {
            ...
            placement = clampStartAvoidingOverlap(siblings, duration, proposedStart)
          }
          nextOffsets[found.id] = timeToPx(placement - start, zoomLevel)
          if (overlayId === element.source.overlayId) {
            previewStartSec = isCrossTrackPreview ? snapped : placement
          }
        }

        setClipDragOffsets(nextOffsets)
        ...
        if (isCrossTrackPreview && audioTrack?.audioTrackId) {
          ...
          setSnapPoint({ time: placementStart, type: 'grid' })
          setClipDragOffsets({
            [element.id]: timeToPx(placementStart - initialStart, zoomLevel),
          })
      ...
      if (element.source.kind === 'overlay') {
        ...
        if (isCrossTrack) {
          ...
          for (const overlayId of overlayIds) {
            ...
            const duration = overlayEl?.duration_sec ?? found.duration
            const siblings =
              liveSession && sourceTrack?.textTrackId
                ? getOverlaySiblingRanges(liveSession, sourceTrack.textTrackId, overlayIds)
                : getTrackSiblingRanges(
