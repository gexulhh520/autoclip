...
export type { CompositionSpec, VisualTransform }
...
/** 与 backend edit_renderer.target_dimensions / scene/canvas 对齐 */
...
export function buildVideoCompositionSpec(
  settings: EditExportSettings,
  sourceWidth: number,
  sourceHeight: number
): CompositionSpec {
  return buildCompositionSpec(settings, sourceWidth, sourceHeight)
}

export function resolveVideoLayerTransforms(
  spec: CompositionSpec
): { foreground: VisualTransform; blurBackdrop?: VisualTransform } {
  const foreground = getForegroundTransform(spec)
  if (spec.background === 'blur') {
    return { foreground, blurBackdrop: getBlurBackdropTransform(spec) }
  }
  return { foreground }
}
