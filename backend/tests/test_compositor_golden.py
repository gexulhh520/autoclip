...
def test_legacy_export_not_gated_by_env(monkeypatch):
    monkeypatch.delenv("AUTOCLIP_EXPORT_LEGACY", raising=False)
    from backend.pipeline.edit_renderer import export_edit_session

    assert callable(export_edit_session)
    assert "Legacy FFmpeg layout export is disabled" not in (export_edit_session.__doc__ or "")
