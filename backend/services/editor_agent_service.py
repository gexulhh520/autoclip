...
30. 用户要按条件找片段（打斗/金句/哲学/共鸣/有感觉的说话等）时：调用 find_block_moments（search_criteria=用户描述，block_id 缺省用 focused/selected）。返回 matches 含 trim 与时间线位置。用户要裁到时间线时用 extract_moment_clips（传入 block_id 与 matches），或 split_block_at_playhead + update_block_trim 精修单段。禁止编造未在 matches 中的时间段。
